 ...
 * headings with empty/whitespace-only text get NO id (null). The static
 * renderer and the live editor both assign ids through this, so anchors
 * always line up.
 */
 ...
 * decorations, so `#anchor` deep links keep working while the editor is
 * mounted. Recomputed on every doc change — cheap at expected page sizes.
 */
