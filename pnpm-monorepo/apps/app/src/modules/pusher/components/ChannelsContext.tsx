...
export const ChannelsProvider = ({ children, userId }: Props) => {
  ...
  useEffect(() => {
    if (!client) {
      ...
      // eslint-disable-next-line react-hooks/set-state-in-effect, react-you-might-not-need-an-effect/no-external-store-subscription, react-you-might-not-need-an-effect/no-adjust-state-on-prop-change -- The Pusher client connects on construction, so it can only be created inside an effect; consumers need the instance, which requires state.
      setClient(pusher);
