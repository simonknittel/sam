...
export const removeExpiredRoles = async () => {
	const citizenWithRoles = await captureAsyncFunc("find citizens with roles", () => prisma.entity.findMany({
		where: {
			roles: {
				not: null,
			},
	...
	const accounts = await captureAsyncFunc("find accounts for citizens", () => prisma.account.findMany({
		where: {
			provider: "discord",
			providerAccountId: {
	...
	const rolesWithMaxAge = await captureAsyncFunc("find roles with max age", () => prisma.role.findMany({
		where: {
			maxAgeDays: {
				not: null,
			},
