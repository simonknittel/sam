---
name: refine-idea
description: Refine a rough idea into an actionable engineering ticket with user story, functional acceptance criteria, non-functional acceptance criteria, and a new GitHub issue.
---

# Refine Idea

Use this skill when you receive a rough idea that is incomplete, vague, or not implementation-ready.

The skill supports feature requests, tasks, bugs, and other technical improvements.

## Goal

Transform a rough idea into a clear, buildable ticket and create a new GitHub issue for it.

## Required Inputs

- Original idea
- Access to the local codebase (for discovery)
- Permission/authentication to create a new GitHub issue (for final step)
...
- Classify idea type: `feature`, `bug`, `task`, `tech-debt`, or `investigation`.
...
### 4) Produce refined ticket(s)

Prepare a refined ticket body using the template below, then create a new GitHub issue for it.
...
# Post issue:
gh issue create --repo <owner/repo> --title "<refined-ticket-title>" --body-file <refined-ticket.md>
```

Discover the current `owner/repo` from the configuration of this repository (`git remote -v`) or ask the user if not available.
