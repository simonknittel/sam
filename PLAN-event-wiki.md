...
- **Uploads follow editability** — superseded in the feedback round: event pages use the wiki's uploadability tiers instead, see "Upload permissions" below.
...
- **Notification**: "Briefing veröffentlicht" fires when the root read scope leaves managers-only for the first time (guarded by a new timestamp on `Event`). Recipients are exactly those who can read the briefing under the new scope, resolved by the notification router from the scope snapshot in the payload (see the phase 7 notes).
...
- **Dialog conventions** (feedback round): the event permissions dialog mirrors `WikiPagePermissionsModal` (same width, headings, icon chips, hints, "Manager" wording) with the scope order Geerbt, Manager, Aufstellung, Teilnehmer, Alle.
...
- **Excluded surfaces**: featured pages, dashboard page, connected pages ("Verknüpfte Seiten"), and all global-wiki listings (search, trash, recents, page targets) exclude event pages. (Export/import was initially excluded too and opened to event managers in the feedback round.)
