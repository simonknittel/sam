...
| No nested grids (2026-08-01) | Grids never nest inside grids. The `wikiGrid` node leaves the `block` group; the containers that host grids (document, `detailsContent`, callout) opt back in via `(block \| wikiGrid)+`, so grid cells (`block+`) reject grids by schema — no insertion path (typing, paste, drag'n'drop) can nest them directly. Indirect nesting (grid → cell → callout/details → grid) is schema-expressible but blocked everywhere it could be triggered: a `grids` flag on `WikiTextRestrictions` (any `wikiGridCell` ancestor) gates the `insertWikiGrid` command and hides/disables the grid entries in the slash menu, gutter palette and toolbar. Grids elsewhere (in callouts/collapsibles, next to each other) stay allowed. |
...
| Text color (2026-08-01) | Custom `wikiTextColor` mark storing a named token (yellow/green/blue/red/purple) instead of Tiptap's TextStyle+Color hex approach — Notion-style, so documents stay independent of the theme's concrete color values (same pattern as `wikiCallout`'s color). Tokens resolve to colors only in CSS (`wikiEditor.css`). Picker in the fixed toolbar and the contextual edit menu. |
...
| Text color | custom `wikiTextColor` mark: named token (yellow/green/blue/red/purple) mapped to theme colors in CSS, keeping documents theme-independent; toolbar picker + swatches in the contextual edit menu |
